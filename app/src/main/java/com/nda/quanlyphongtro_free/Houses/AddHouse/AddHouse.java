...
public class AddHouse extends AppCompatActivity {
    ...
    TextInputEditText textInputEdt_getBaoTruocNgayChuyen;
    ...
    private void init()
    {
        ...
        formatMoneyType(textInputEdt_getDientich);
    ...
    private void executeAddHouse() {
        ...
        else if(strQuanHuyen.equals(""))
        {
            ...
            return;
        } else if(edtDienTich.equals(""))
        {
            Toast.makeText(this, "Error: Nhập diện tích", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (edtDienTich.contains(","))
            edtDienTich = edtDienTich.replaceAll(",","");
...
//        myRef.child("houses").child(strTinhThanhPho).child(houseId).setValue(houses);
    ...
    private void dialogAddImage() {
        AdapterAddImage adapter = new AdapterAddImage(this, imageUrlList);
